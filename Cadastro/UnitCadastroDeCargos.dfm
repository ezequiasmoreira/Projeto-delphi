inherited FormCadastroCargos: TFormCadastroCargos
  Caption = 'Cadastro de cargos'
  ClientHeight = 170
  ClientWidth = 501
  ExplicitWidth = 507
  ExplicitHeight = 199
  PixelsPerInch = 96
  TextHeight = 13
  inherited ButtonPesquisar: TSpeedButton
    Left = 119
    Top = 12
    Height = 23
    OnClick = ButtonPesquisarClick
    ExplicitLeft = 119
    ExplicitTop = 12
    ExplicitHeight = 23
  end
  object Label1: TLabel [1]
    Left = 31
    Top = 16
    Width = 33
    Height = 13
    Caption = 'C'#243'digo'
    FocusControl = EditCodigo
  end
  object Label2: TLabel [2]
    Left = 155
    Top = 16
    Width = 27
    Height = 13
    Caption = 'Nome'
    FocusControl = EditNome
  end
  object Label3: TLabel [3]
    Left = 24
    Top = 39
    Width = 40
    Height = 13
    Caption = 'Fun'#231#245'es'
    FocusControl = EditFuncao
  end
  object Label4: TLabel [4]
    Left = 339
    Top = 16
    Width = 60
    Height = 13
    Caption = 'Sal'#225'rio inicial'
  end
  object Label5: TLabel [5]
    Left = 2
    Top = 62
    Width = 62
    Height = 13
    Caption = 'C'#243'd. depart.'
    FocusControl = EditCodDepartamento
  end
  object Label6: TLabel [6]
    Left = 151
    Top = 62
    Width = 113
    Height = 13
    Caption = 'Nome do departamento'
    FocusControl = EditNomeDepartamento
  end
  object ButtonPesquisarDepartamento: TSpeedButton [7]
    Left = 119
    Top = 58
    Width = 30
    Height = 23
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    OnClick = ButtonPesquisarDepartamentoClick
  end
  inherited GroupBox1: TGroupBox
    Top = 84
    Width = 501
    ExplicitTop = 84
    ExplicitWidth = 501
    inherited DBNavigator: TDBNavigator
      Width = 497
      Hints.Strings = ()
      ExplicitWidth = 497
    end
  end
  object EditCodigo: TDBEdit [9]
    Left = 73
    Top = 13
    Width = 46
    Height = 21
    CharCase = ecUpperCase
    DataField = 'CD_CARGOS'
    DataSource = ds
    Enabled = False
    ReadOnly = True
    TabOrder = 1
  end
  object EditNome: TDBEdit [10]
    Left = 186
    Top = 13
    Width = 150
    Height = 21
    CharCase = ecUpperCase
    DataField = 'NM_CARGOS'
    DataSource = ds
    TabOrder = 2
  end
  object EditFuncao: TDBEdit [11]
    Left = 73
    Top = 36
    Width = 426
    Height = 21
    CharCase = ecUpperCase
    DataField = 'FUNCAO'
    DataSource = ds
    TabOrder = 3
  end
  object EditCodDepartamento: TDBEdit [12]
    Left = 73
    Top = 59
    Width = 46
    Height = 21
    CharCase = ecUpperCase
    DataField = 'DEPARTAMENTO_CD_DEPARTAMENTO'
    DataSource = ds
    TabOrder = 4
  end
  object EditNomeDepartamento: TDBEdit [13]
    Left = 266
    Top = 59
    Width = 233
    Height = 21
    CharCase = ecUpperCase
    DataField = 'NM_DEPARTAMENTO'
    DataSource = ds
    TabOrder = 5
  end
  object EditSalarioInicial: TDBEdit [14]
    Left = 405
    Top = 13
    Width = 94
    Height = 21
    DataField = 'SAL_INICIAL'
    DataSource = ds
    TabOrder = 6
  end
  inherited Query: TIBQuery
    SQL.Strings = (
      'SELECT CARGOS.*, DEPARTAMENTO. NM_DEPARTAMENTO'
      ''
      'FROM CARGOS'
      ''
      'INNER JOIN DEPARTAMENTO'
      ''
      
        'ON (DEPARTAMENTO. CD_DEPARTAMENTO = CARGOS. DEPARTAMENTO_CD_DEPA' +
        'RTAMENTO)'
      ''
      'ORDER BY NM_CARGOS')
    Left = 424
    Top = 48
    object QueryCD_CARGOS: TIntegerField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'CD_CARGOS'
      Origin = '"CARGOS"."CD_CARGOS"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QueryNM_CARGOS: TIBStringField
      DisplayLabel = 'Nome'
      FieldName = 'NM_CARGOS'
      Origin = '"CARGOS"."NM_CARGOS"'
      Required = True
      Size = 100
    end
    object QueryFUNCAO: TIBStringField
      DisplayLabel = 'Fun'#231#227'o'
      FieldName = 'FUNCAO'
      Origin = '"CARGOS"."FUNCAO"'
      Required = True
      Size = 100
    end
    object QueryDEPARTAMENTO_CD_DEPARTAMENTO: TIntegerField
      DisplayLabel = 'C'#243'digo do departamento'
      FieldName = 'DEPARTAMENTO_CD_DEPARTAMENTO'
      Origin = '"CARGOS"."DEPARTAMENTO_CD_DEPARTAMENTO"'
      Required = True
    end
    object QueryNM_DEPARTAMENTO: TIBStringField
      DisplayLabel = 'Nome do departamento'
      FieldName = 'NM_DEPARTAMENTO'
      Origin = '"DEPARTAMENTO"."NM_DEPARTAMENTO"'
      Required = True
      Size = 100
    end
    object QuerySAL_INICIAL: TIBBCDField
      FieldName = 'SAL_INICIAL'
      Origin = '"CARGOS"."SAL_INICIAL"'
      DisplayFormat = '#,##0.00'
      Precision = 15
      Size = 2
    end
  end
  inherited Update: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select '
      '  CD_CARGOS,'
      '  NM_CARGOS,'
      '  FUNCAO,'
      '  SAL_INICIAL,'
      '  DEPARTAMENTO_CD_DEPARTAMENTO'
      'from CARGOS '
      'where'
      '  CD_CARGOS = :CD_CARGOS')
    ModifySQL.Strings = (
      'update CARGOS'
      'set'
      '  CD_CARGOS = :CD_CARGOS,'
      '  NM_CARGOS = :NM_CARGOS,'
      '  FUNCAO = :FUNCAO,'
      '  SAL_INICIAL = :SAL_INICIAL,'
      '  DEPARTAMENTO_CD_DEPARTAMENTO = :DEPARTAMENTO_CD_DEPARTAMENTO'
      'where'
      '  CD_CARGOS = :OLD_CD_CARGOS')
    InsertSQL.Strings = (
      'insert into CARGOS'
      
        '  (CD_CARGOS, NM_CARGOS, FUNCAO, SAL_INICIAL, DEPARTAMENTO_CD_DE' +
        'PARTAMENTO)'
      'values'
      
        '  (:CD_CARGOS, :NM_CARGOS, :FUNCAO, :SAL_INICIAL, :DEPARTAMENTO_' +
        'CD_DEPARTAMENTO)')
    DeleteSQL.Strings = (
      'delete from CARGOS'
      'where'
      '  CD_CARGOS = :OLD_CD_CARGOS')
  end
end
